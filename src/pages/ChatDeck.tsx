...
export const ChatDeck: React.FC<ChatDeckProps> = ({
  ...
  const [messages, setMessages] = React.useState<Message[]>([
    ...
    { id: "2", sender: "X_Matrix", text: "O sinal está forte hoje em Neo-Tokyo.", timestamp: new Date(), isMe: false },
    { id: "3", sender: "CyberRunner_23", text: "Confirmado. Recebendo pacotes 0x0C sem delay.", timestamp: new Date(), isMe: true, status: "sent" },
  ]);
  ...
  // DJ State
  const [isDeckOpen, setIsDeckOpen] = React.useState(false);
  const [radioName, setRadioName] = React.useState("CyberRunner_FM");
  const [radioLogo, setRadioLogo] = React.useState("https://picsum.photos/seed/dj/200/200");

  React.useEffect(() => {
    if (selectedRadio) {
      setRadioName(selectedRadio.name);
      setRadioLogo(selectedRadio.image);
    }
  }, [selectedRadio]);
  const [carouselPhotos, setCarouselPhotos] = React.useState([
  ...
  const [playlist, setPlaylist] = React.useState<string[]>([
    "Neon Nights - Cyber_Ghost",
    "Mesh Pulse - Gossip_V3",
    "Aether Drift - Orion_9",
    "Data Stream - X_Matrix"
  ]);
